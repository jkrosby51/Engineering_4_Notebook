...
while True:
    ...
    for i in range(len(timelist)-1): #is that syntax correct for range()? -------------------------CHECK
        ...
        line = Line(xPixel+timelist[i], yPixel+altlist[i], xPixel+timelist[i+1], yPixel+timelist[i+1], color=0x00FF00) #feel free to change the color ---------CHECK
        #theres more to add. Ill do it later -------------------------CHECK
